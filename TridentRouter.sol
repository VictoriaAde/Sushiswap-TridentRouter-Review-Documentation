...
contract TridentRouter is ITridentRouter, SelfPermit, Multicall {
    ...
    function burnLiquidity(
        ...
        for (uint256 i = 0; i < n; i = _increment(i)) {
            if (minWithdrawals[i].token != withdrawnLiquidity[i].token)
                revert IncorrectSlippageParams();
            if (withdrawnLiquidity[i].amount < minWithdrawals[i].amount)
                revert TooLittleReceived();
        }
    }
    ...
    function sweep(
        ...
        if (fromBento) {
            ...
            uint256 amount = token == USE_NATIVE
